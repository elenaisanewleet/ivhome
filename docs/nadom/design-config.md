...
2. `docs/nadom/references/design/nadom-design-package-v6.2.html` — current active confirmed visual/design baseline when the file exists in the repo or is explicitly attached to the task.
...
4. `docs/nadom/references/design/nadom-design-package-v6.html` — stable product UI continuity source.
5. `docs/nadom/references/design/nadom-design-package-v5.html` — supporting design reference for strong visual/product ideas that still fit the current package.
6. `docs/nadom/references/design/nadom-design-package-v4.html` — supporting design reference for earlier strong foundation and decisions that were not intentionally replaced.
...
8. Older design packages v1/v2/v3, if present — historical support only. Do not let them override v4–v6.2.
9. `docs/nadom/references/design/nadom-workflow-board.png` — workflow completeness checklist only.

If sources conflict, follow the priority above: the canonical TOV owns wording and tone, and v6.2 is the current active confirmed visual/design baseline when available or explicitly attached. If v6.2 / v6.1 / v6 are not present in the repo or attached to the task, agents must say they are unavailable and use the next available attached or repo source. Do not invent missing design files, placeholder files, or a new visual style outside this lineage.

## Current availability note

`nadom-design-package-v6.3.html` is the current target / next package to be created from the v4 → v6.2 lineage. Do not treat it as available until the file exists in the repository or is explicitly attached to the task. If v6.2 / v6.1 / v6 or any other referenced visual file is unavailable in the working tree and not attached, use the next available attached or repo source and note the limitation.

## Final design formula

* v6.3 = current target / next package to be created.
* v6.2 = current active confirmed design baseline when available or explicitly attached.
* v6.1 = nearest previous baseline.
* v6 = stable product UI continuity source.
* v5 = supporting visual/product ideas.
* v4 = supporting strong foundation.
* v6.2 owns the current confirmed screens, palette application, UI components, Telegram assets, bot assets, banners, clinic/medservice cards, admin cabinet, partner cabinet, motion examples, and export-ready visuals until v6.3 exists in the repo and is accepted as the active current design package.
* Once v6.3 exists in the repo, it can become the active current design package.
...
Use v6.2 as the current active confirmed visual/design baseline when it exists in the repo or is explicitly attached:
...
Use v6 as the stable product UI continuity source when v6.2/v6.1 are unavailable or silent.
...
* Preserve the best working parts across v4 → v6.2, but v6.2 wins when there is a direct conflict until v6.3 exists in the repo and is accepted as the active current design package.
...
Follow the current active confirmed v6.2 medservice-card direction when v6.2 exists in the repo or is explicitly attached. Use v6.1/v6/v5/v4 only for continuity and supporting ideas that still fit v6.2.
