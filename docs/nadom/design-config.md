...
2. `docs/nadom/references/design/nadom-design-package-v6.2.html` — current active confirmed visual/design baseline.
...
If sources conflict, follow the priority above: the canonical TOV owns wording and tone, and v6.2 is the current active confirmed visual/design baseline. If a referenced visual file is missing, state that it is unavailable and use the next available source. Do not invent placeholder files or a new visual style outside this lineage.

## Current availability note

`nadom-design-package-v6.3.html` is the current target / next package to be created from the v4 → v6.2 lineage. Do not treat it as available until the file exists in the repository or is explicitly attached to the task. If any referenced visual file is unavailable in the working tree, use the next available source and note the limitation.

## Final design formula

* v6.3 = current target / next package to be created.
* v6.2 = current active confirmed design baseline.
...
Use v6.2 as the current active confirmed visual/design baseline when available:
...
Follow the current active confirmed v6.2 medservice-card direction when available. Use v6.1/v6/v5/v4 only for continuity and supporting ideas that still fit v6.2.
